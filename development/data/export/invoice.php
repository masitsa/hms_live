...
<div id="header" class="container">
    	<div id="logo">
	    <h1><a href="#">SUMC</a></h1>
			<p><a href="http://www.strathmore.edu">invoice</a></p>
		</div>
	</div>
	<!-- end #header -->
	<div class="row-fluid">
		...
		for($s = $current_item; $s < $end_item; $s++){
			...
			$invoice_id = mysql_result($rs, $s,  "visit_id"); // A unique Invoice ID which can be used to locate the Invoices from your System

            $patient_id = mysql_result($rs, $s, "patient_id"); //The Patients Unique Identity Number
            $service_id = mysql_result($rs, $s, "service_id"); //A unique ID for distinguishing the Service being invoices
            $service_charge = mysql_result($rs, $s, "visit_charge_amount"); //Amount Invoiced
            $visit_charge_time = mysql_result($rs, $s, "visit_charge_timestamp"); //Date and Time of the transaction
            $service_charge_name = mysql_result($rs, $s, "service_charge_name"); //DESCRIPTION of the Invoice Payment
		
		?>
        <tr>
        	<td><?php echo ($s+1)?></td>
        	<td><?php echo $invoice_id?></td>
        	<td><?php echo $visit_charge_time?></td>
