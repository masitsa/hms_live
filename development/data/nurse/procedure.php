...
$sql = "INSERT INTO visit_procedure(procedure_id , visit_id, units) VALUES ('$procedure_id','$v_id',$suck)";
//echo $sql;
$save = new Database();
$save->insert($sql);


// $sql = "INSERT INTO visit_charge(visit_id, service_charge_id, visit_charge_amount) VALUES ($v_id, (SELECT service_charge_id FROM service_charge WHERE visit_type_id = (SELECT visit_type FROM visit WHERE visit_id = $v_id) AND service_charge_name = (SELECT procedures FROM `procedure` WHERE procedure_id = $procedure_id)), (SELECT service_charge_amount FROM service_charge WHERE visit_type_id = (SELECT visit_type FROM visit WHERE visit_id = $v_id) AND service_charge_name = (SELECT procedures FROM `procedure` WHERE procedure_id = $procedure_id)))";//echo $sql;
// $save = new Database();
// $save->insert($sql);
...
$sqlv = "INSERT INTO visit_charge(visit_id, service_charge_id,visit_charge_amount,visit_charge_units) VALUES ('$v_id','$procedure_id', (select service_charge_amount from service_charge where service_charge_id=".$procedure_id."),$suck)";
//echo $sqlv;
$savev = new Database();
$savev->insert($sqlv);
