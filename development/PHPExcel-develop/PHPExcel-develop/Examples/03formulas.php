...
$service_id_e=mysql_result($rss,$s1, "service_id");
 if($k<=$num_rowss){ 
 if($s1==1){
  $objPHPExcel->getActiveSheet()->setCellValue($v.$s1, $service_name);
 }
 else{
	 
	
  $objPHPExcel->getActiveSheet()->setCellValue('B2', 3)
                            	  ->setCellValue('B3', 7)
                            	  ->setCellValue('B4', 13);  
   // echo "$k $v \n";
} }
...
echo 'Call time to wrie Workbook was ' , sprintf('%.4f',$callTime) , " seconds" , EOL;
// Echo memory usage
echo date('H:i:s') , ' Current memory usage: ' , (memory_get_usage(true) / 1024 / 1024) , " MB" , EOL;
