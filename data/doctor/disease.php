...
if($_REQUEST['search']){
	...
	$get = new doctor();
	$rs = $get->search_disease($search, $order);
	$num_rows = mysql_num_rows($rs);
	...
	if($num_rows == 0){
		...
		<script type="text/javascript">
			...
			window.location.href = "disease.php?visit_id=<?php echo $visit_id?>";
		</script>
		<?php
	}
}
	//paginate the items
	$pages1 = intval($num_rows/15);
	$pages2 = $num_rows%(2*15);
	...
	if($current_page == 1){
		...
		$current_item = ($current_page-1) * 15;
	}
...
<body onLoad="get_disease(<?php echo $visit_id;?>)">
  <div id="header" class="container">
    	<div id="logo">
	    <h1><a href="#">SUMC</a></h1>
			<p><a href="http://www.strathmore.edu">Diseases</a></p>
		</div>
	</div>
	<!-- end #header -->
	<div class="row-fluid">
    	...
    	<table align="center" border="0">
        	<tr>
                    <td>
                        <form class="form-search" action="disease.php"  method="post" >
    	...
    	<table align="center" border="0">
        	<tr>
                    <td>
                        <form class="form-search" action="disease.php"  method="post" >
                            <a href='disease.php?visit_id=<?php echo $visit_id;?>'><input type="button" class="btn btn-primary btn-large" value="Close Search"/></a>
    					</form>
                    </td>
            </tr>
        </table>
        ...
        <form action="disease.php" method="POST">
        	<table class='table table-striped table-hover table-condensed'>
            	<thead>
                	<td></td>
                	<th><a href="disease.php?order=diseases_code&search=<?php echo $search?>&id=<?php echo $current_page?>">Code</a></th>
                	<th><a href="disease.php?order=diseases_name&search=<?php echo $search?>&id=<?php echo $current_page?>">Name</a></th>
                </thead>
        <?php 
		for($s = $current_item; $s < $end_item; $s++){
		
			$name = mysql_result($rs, $s, "diseases_name");
			$code = mysql_result($rs, $s, "diseases_code");
			$id = mysql_result($rs, $s, "diseases_id");
