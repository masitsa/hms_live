...
class Physiotherapy extends auth
{	
	function __construct()
	{
		parent:: __construct();
		$this->load->model('physiotherapy_model');
	...
	public function index()
	{
		...
		$data['sidebar'] = 'physio_sidebar';
		$this->load->view('auth/template_sidebar', $data);	
	}
	
	public function physiotherapy_queue($page_name = NULL)
	{
		// this is it
		...
		$data['title'] = 'Physiotherapy Queue';
		$v_data['title'] = 'Physiotherapy Queue';
		$v_data['module'] = 1;
		
		$v_data['type'] = $this->reception_model->get_types();
		$v_data['doctors'] = $this->reception_model->get_doctor();
		
		$data['content'] = $this->load->view('physiotherapy_queue', $v_data, true);
	...
	public function search_physiotherapy_billing($visit_id)
	{
		...
		$this->physiotherapy_services($visit_id);
	}
	public function close_physiotherapy_billing_search($visit_id)
	{
		$this->session->unset_userdata('billing_search');
		$this->physiotherapy_services($visit_id);
	}
	function physiotherapy_services($visit_id)
	{
		...
		$where = 'service_id = 8 AND visit_type_id = '.$visit_t;
		...
		//pagination
		$this->load->library('pagination');
		$config['base_url'] = site_url().'/physiotherapy/physiotherapy_services/'.$visit_id;
		$config['total_rows'] = $this->reception_model->count_items($table, $where);
	...
	public function send_to_accounts($visit_id)
	{
		...
		{
				redirect("physiotherapy/physiotherapy_queue");
		}
		else
		{
			echo 'error';
