...
foreach ($rs as $key6):
	...
	{
		$button = "<a class='btn btn-danger' href='".site_url()."/laboratory/remove_cost/".$visit_charge_response."/".$visit_id."' onclick='return confirm('Do you want to remove ".$test." from the invoice ?');'>Remove lab test charge</a>";
		$status = '';
	}
	else
	{
		$button = "<a class='btn btn-success' onclick='return confirm('Do you want to add ".$test." to the invoice ?');' href='".site_url()."/laboratory/add_lab_cost/".$service_charge_id."/".$visit_id."' >Charge for lab test</a>";
		$status ="<a class='btn btn-info' href='".site_url()."/laboratory/remove_lab_test/".$service_charge_id."/".$visit_id."' >Remove from list</a>";
	}
