...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_hidden_input($field_types,$value)
	{
		if($field_types->extras !== null && $field_types->extras != false)
			$value = $field_types->extras;
		$input = "<input id='field-{$field_types->name}' type='hidden' name='{$field_types->name}' value='$value' />";
		return $input;		
	}
